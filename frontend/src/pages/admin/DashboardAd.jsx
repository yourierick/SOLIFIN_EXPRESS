...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (isSuperAdmin && !loadingPermissions) {
      ...
      // Initialiser les données par défaut si l'utilisateur n'est pas super admin
      setDashboardData({
        cards: {
          total_users: 0,
          today_users: 0,
          total_withdrawals: 0,
          failed_commissions: 0
        },
        network_overview: {
          active_users: 0,
          inactive_users: 0,
  ...
  return (
    <div className={`${themeColors.background} min-h-screen p-6 md:p-8`}>
      {/* Affichage du Dashboard pour tous les utilisateurs (avec données par défaut si non admin) */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10 transition-all duration-300">
        {loading
          ...
          : // Afficher les données par défaut (vides/0) si pas de permissions
            [
              {
                name: "Utilisateurs totaux",
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-3 h-10">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart
                        data={[]} // Données vides pour le graphique
                        margin={{ top: 0, right: 0, left: 0, bottom: 0 }}
                      >
                        <Area
                          type="monotone"
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                ...
                >
                  {formatNumber(dashboardData?.network_overview?.active_users || 0)}
                </p>
              </div>
              <div
                className={`p-4 rounded-lg ${
              ...
              >
                ...
                >
                  {formatNumber(dashboardData?.network_overview?.inactive_users || 0)}
                </p>
              </div>
              <div
              ...
              >
                <div className="flex justify-between items-center mb-2">
                  <p className={`text-sm ${themeColors.text.muted}`}>
                    Nouveaux membres ({dashboardData?.network_overview?.period || "aujourd'hui"})
                  </p>
                  <p
                  ...
                  >
                    {formatNumber(dashboardData?.network_overview?.new_users || 0)}
                  </p>
                </div>
          ...
          <div>
            ...
            >
              <ul className="divide-y divide-gray-200 dark:divide-gray-600">
                {(dashboardData?.network_overview?.top_referrers || []).slice(0, 5)
                  .map((referrer, index) => (
                    <li
                      key={index}
                    ...
                    >
                      <div className="flex items-center">
                        {referrer.profile_photo ? (
                          ...
                          <div className={`flex items-center justify-center w-8 h-8 rounded-full mr-3 ${isDarkMode ? "bg-gray-600 text-gray-300" : "bg-gray-300 text-gray-700"}`}>
                            <div className="flex items-center">
                              {referrer.profile_photo ? (
                                <img
                                  src={referrer.profile_photo}
