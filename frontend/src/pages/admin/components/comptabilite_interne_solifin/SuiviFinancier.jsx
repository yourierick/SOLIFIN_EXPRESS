...
const SuiviFinancier = React.memo(() => {
  ...
  const transactionTypes = [
    ...
    },
    {
      ...
      description: 'Suivi des ventes de boost de publication',
      icon: BoostSaleIcon,
      color: 'info',
    },
    {
    ...
    {
      key: 'virtual_send',
      label: 'Envoi de virtuels',
      description: 'Suivi des envois de virtuels solifin',
      icon: VirtualSaleIcon,
      color: 'secondary',
    },
    {
    ...
    },
    {
      ...
      label: 'Commission sur retrait des fonds',
      description: 'Suivi des commissions solifin sur les retraits',
      icon: WithdrawalCommissionIcon,
      color: 'warning',
    },
    {
      key: 'transfer_commission',
      label: 'Commission sur les transferts des fonds',
      description: 'Suivi des commissions solifin sur les transferts',
      icon: TransferCommissionIcon,
      color: 'info',
    },
    {
      key: 'sale_commission',
      label: 'Commission sur la vente',
      description: 'Suivi des commissions solifin sur les ventes',
      icon: TransferIcon,
      color: 'secondary',
    },
    {
      key: 'esengo_funds_transfer',
      label: 'Paiement des fonds esengo',
      description: 'Suivi des paiements des fonds esengo',
    ...
    },
    {
      ...
      description: 'Suivi des ajustements de solde',
      icon: AnalyticsIcon,
      color: 'default',
    },
    {
    ...
    },
    {
      ...
      label: 'Retrait SOLIFIN',
      description: 'Suivi des retraits SOLIFIN',
      icon: WithdrawalIcon,
      color: 'error',
    }
  ];
  ...
  const handleExport = async (exportType) => {
    try {
      ...
      const response = await axios.get('/api/admin/solifin-financial-transactions/export', {
        params: params
      });
      ...
      const headers = [
        ...
        'Statut',
        'Déscription',
        'Solde marchand avant',
        'Solde marchand après',
        'Engagement utilisateurs avant',
        'Engagement utilisateurs après',
        'Profits solifin avant',
        'Profits solifin après',
        'Traité par',
        'Traité le',
        'Raison (rejet, echec, ...)',
        'Date de création',
      ...
      const rows = data.map(item => [
        ...
        item.engagement_users_before,
        item.engagement_users_after,
        item.plateforme_benefices_before,
        item.plateforme_benefices_after,
        item.processor,
        item.processed_at,
      ...
      const colWidths = [
        ...
        { wch: 15 }, // Statut
        { wch: 50 }, // Déscription
        { wch: 20 }, // Solde marchand avant
        { wch: 20 }, // Solde marchand après
        { wch: 20 }, // Engagement utilisateurs avant
        { wch: 20 }, // Engagement utilisateurs après
        { wch: 20 }, // Profits solifin avant
        { wch: 20 }, // Profits solifin après
        { wch: 30 }, // Traité par
        { wch: 15 }, // Traité le
        ...
        <Box sx={{ 
          ...
          <Box sx={{ display: 'flex', gap: 1 }}>
            <Button
              ...
              startIcon={isRefreshing ? <div className="animate-spin" /> : <RefreshIcon />}
              sx={{
                px: { xs: 2, sm: 3 },
                py: { xs: 1, sm: 1.5 },
                ...
                background: isDarkMode 
                  ...
                  : 'linear-gradient(135deg, rgba(34, 197, 94, 0.9) 0%, rgba(22, 163, 74, 0.9) 100%)',
                color: 'white',
                border: `1px solid ${isDarkMode ? 'rgba(34, 197, 94, 0.6)' : 'rgba(34, 197, 94, 0.6)'}`,
                backdropFilter: 'blur(10px)',
                transition: 'all 0.3s ease',
                '&:hover': {
                  background: isDarkMode 
                    ? 'linear-gradient(135deg, rgba(22, 163, 74, 0.9) 0%, rgba(16, 185, 129, 0.9) 100%)' 
                    : 'linear-gradient(135deg, rgba(22, 163, 74, 0.9) 0%, rgba(16, 185, 129, 0.9) 100%)',
                    ...
                    allTransactions.map((transaction) => (
                      ...
                      >
                        ...
                        <TableCell>
                          ...
                          >
                            {formatAmount(transaction.solde_marchand_before)} $
                          </Typography>
                        </TableCell>
                        <TableCell>
                          <Typography 
                          ...
                          >
                            {formatAmount(transaction.solde_marchand_after)} $
                          </Typography>
                        </TableCell>
                        <TableCell>
                          <Typography 
                            ...
                            fontWeight={600}
                            sx={{ 
                              color: `${isDarkMode ? 'rgba(255, 70, 70, 1)':'rgba(246, 59, 59, 1)'}`,
                              fontSize: '0.9rem'
                            }}
                          >
                            {formatAmount(transaction.engagement_users_before)} $
                          </Typography>
                        </TableCell>
                        <TableCell>
                          <Typography 
                          ...
                          >
                            {formatAmount(transaction.engagement_users_after)} $
                          </Typography>
                        </TableCell>
                        <TableCell>
                          <Typography 
                            ...
                            fontWeight={600}
                            sx={{ 
                              color: `${isDarkMode ? 'rgba(255, 129, 51, 1)':'rgba(255, 135, 23, 1)'}`,
                              fontSize: '0.9rem'
                            }}
                          >
                            {formatAmount(transaction.plateforme_benefices_before)} $
                          </Typography>
                        </TableCell>
                        <TableCell>
                          <Typography 
                          ...
                          >
                            {formatAmount(transaction.plateforme_benefices_after)} $
                          </Typography>
                        </TableCell>
                        <TableCell>
                          <Typography 
                  ...
                  <List sx={{ p: 0 }}>
                    ...
                    <ListItem sx={{ p: 0, mb: 1 }}>
                      <ListItemText
                        ...
                        secondary={
                          <Chip
                            label={selectedTransaction?.flow === 'in' ? 'Entrée' : 'Sortie'}
                            color={selectedTransaction?.flow === 'in' ? 'success' : 'error'}
                            size="small"
                            variant="outlined"
                          />
                        }
                    ...
                    <ListItem sx={{ p: 0, mb: 1 }}>
                      <ListItemText
                        ...
                        secondary={
                          <Chip
                            label={selectedTransaction?.nature === 'internal' ? 'Interne' : 'Externe'}
                            color={selectedTransaction?.nature === 'internal' ? 'primary' : 'secondary'}
                            size="small"
                            variant="outlined"
                          />
                        }
                    ...
                    <ListItem sx={{ p: 0, mb: 1 }}>
                      <ListItemText
                        primary="Soldes Marchand"
                        secondary={
                          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                            <Typography variant="body2" sx={{ fontSize: '0.85rem' }}>
                              Avant: <strong>{formatAmount(selectedTransaction?.solde_marchand_before)}</strong>
                            </Typography>
                            <Typography variant="body2" sx={{ fontSize: '0.85rem' }}>
                              Après: <strong>{formatAmount(selectedTransaction?.solde_marchand_after)}</strong>
                            </Typography>
                          </Box>
                        }
                        primaryTypographyProps={{ 
                    ...
                    <ListItem sx={{ p: 0, mb: 1 }}>
                      <ListItemText
                        primary="Engagements Utilisateurs"
                        secondary={
                          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                            <Typography variant="body2" sx={{ fontSize: '0.85rem' }}>
                              Avant: <strong>{formatAmount(selectedTransaction?.engagement_users_before)}</strong>
                            </Typography>
                            <Typography variant="body2" sx={{ fontSize: '0.85rem' }}>
                              Après: <strong>{formatAmount(selectedTransaction?.engagement_users_after)}</strong>
                            </Typography>
                          </Box>
                        }
                        primaryTypographyProps={{ 
                    ...
                    <ListItem sx={{ p: 0, mb: 1 }}>
                      <ListItemText
                        primary="Bénéfices Plateforme"
                        secondary={
                          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                            <Typography variant="body2" sx={{ fontSize: '0.85rem' }}>
                              Avant: <strong>{formatAmount(selectedTransaction?.plateforme_benefices_before)}</strong>
                            </Typography>
                            <Typography variant="body2" sx={{ fontSize: '0.85rem' }}>
                              Après: <strong>{formatAmount(selectedTransaction?.plateforme_benefices_after)}</strong>
                            </Typography>
                          </Box>
                        }
                        primaryTypographyProps={{ 
